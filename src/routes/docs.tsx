	...
	head: () => ({
		meta: [
			{ title: "Sigil — Integration Docs" },
			{
				name: "description",
				content:
					"Integrate Sigil into your QUBIC app. Build the URI, open it, listen for the callback.",
			},
		],
	}),
	component: DocsPage,
		...
		<div className="docs-shell">
			...
			<main className="docs-main">
				...
				<div className="docs-content">
					<div id="intro" className="docs-eyebrow">
						[ DOCS · v1 · 2026 ]
					</div>
					<h1>
					...
					<p>
						Sigil is a desktop wallet. Your frontend asks it to sign things over
						a <code className="inline">sigil://</code> URI. Sigil pops up, the
						user reviews and approves, then Sigil POSTs the signed result to a
						callback URL you control. That's the whole interface.
					</p>
					<p>
						...
						SDK — it builds envelopes, signs them, and parses callback
						responses. Or build the URI yourself: the protocol is small enough
						to drop into any JS, Python, or shell context.
					</p>

					<div className="callout info">
						<div className="callout-tag">[ SOURCE OF TRUTH ]</div>
						<p>
							Everything below is taken from the Rust validator at{" "}
							<code className="inline">src-tauri/src/deep_link.rs</code>. If the
							code and these docs disagree, the code wins.
						</p>
					</div>
					...
					<p>
						One URI, one callback. Sigil registers the{" "}
						<code className="inline">sigil://</code> scheme with the OS; opening
						such a URI brings Sigil to the foreground with your request.
					</p>
					<pre className="doc-pre">
						<span className="pre-label">URI SHAPE</span>
						<span className="t-kw">sigil</span>://
						<span className="t-kw">v1</span>/
						<span className="t-kw">request</span>?
						<span className="t-key">d</span>={"<"}
						<span className="t-com">base64url-encoded JSON envelope</span>
						{">"}
						{"\n"} [&amp;<span className="t-key">cb</span>={"<"}
						<span className="t-com">optional HTTPS callback URL</span>
						{">"}]
					</pre>
					<ul className="brief">
						<li>
							Scheme must be <code className="inline">sigil</code>
						</li>
						<li>
							Host must be <code className="inline">v1</code>, path must be{" "}
							<code className="inline">/request</code>
						</li>
						<li>
							<code className="inline">d</code> = base64url (no padding) of a
							JSON envelope:{" "}
							<code className="inline">{"{ request, callback?, proof? }"}</code>
						</li>
						<li>
							Callback can be set in the envelope's{" "}
							<code className="inline">callback</code> field or via the{" "}
							<code className="inline">&amp;cb=</code> query param — if both are
					...
					<ol style={{ paddingLeft: 20, lineHeight: 1.7 }}>
						<li>
							<strong>Build your JSON envelope</strong>:{" "}
							<code className="inline">
								{"{ request: { type, nonce, dapp, ...fields }, callback? }"}
							</code>
						</li>
						<li>
							<strong>Base64url-encode the envelope</strong> (no padding) and
							put it in <code className="inline">?d=</code>.
						</li>
						<li>
							<strong>Spin up a callback endpoint</strong> if you want a
							structured response — put its HTTPS URL in the envelope's{" "}
							<code className="inline">callback</code> field.
						</li>
						<li>
							<strong>Open the URI</strong> —{" "}
							<code className="inline">window.location.href = uri</code> in the
							browser, <code className="inline">open</code>/
							<code className="inline">xdg-open</code>/
							<code className="inline">start</code> from a native app.
						</li>
						<li>
							<strong>Sigil validates</strong> the URI in Rust, focuses its
							window, queues the request.
						</li>
						<li>
							<strong>User approves or rejects</strong>. Sigil POSTs the result
							to your callback from the Rust layer.
						</li>
					</ol>
					...
					<p>
						The <code className="inline">d</code> parameter encodes a JSON
					...
					<p>
						...
						<strong>URL-safe base64 without padding</strong>.
					</p>
					<pre className="doc-pre">
						<span className="pre-label">JAVASCRIPT</span>
						<span className="t-kw">function</span>{" "}
						<span className="t-key">b64url</span>(str) {"{"}
						<span className="t-kw">return</span>{" "}
						<span className="t-key">btoa</span>(str) .replaceAll(
						<span className="t-str">'+'</span>,{" "}
						<span className="t-str">'-'</span>) .replaceAll(
						<span className="t-str">'/'</span>,{" "}
						<span className="t-str">'_'</span>) .replaceAll(
						<span className="t-str">'='</span>,{" "}
						<span className="t-str">''</span>);
						{"}"}
						<span className="t-kw">function</span>{" "}
						<span className="t-key">buildSigilUri</span>(request, callback){" "}
						{"{"}
						<span className="t-com">
							{
								"// Wrap request in the envelope — callback lives here, not as a query param"
							}
						</span>
						<span className="t-kw">const</span> envelope = {"{"} request,
						callback: callback ?? <span className="t-kw">null</span> {"};"}
						<span className="t-kw">const</span> d ={" "}
						<span className="t-key">b64url</span>(
						<span className="t-key">JSON</span>.stringify(envelope));
						<span className="t-kw">return</span>{" "}
						<span className="t-str">
							`sigil://v1/request?d=${"{"}d{"}"}`
						</span>
						;{"}"}
					</pre>
					...
					<div className="callout info" style={{ marginTop: 32 }}>
						...
						<p>
							...
							<code className="inline">legacy_unverified</code> — metadata is
							self-reported) or carry an ES256{" "}
							<code className="inline">proof</code> signed by a registered dApp
							issuer. If a proof is present but the signature is invalid, or the
							issuer is in the registry but the origin doesn't match,{" "}
							<strong>Sigil blocks approval</strong>. Unsigned requests can
							still be reviewed and approved — the UI shows the trust level
							clearly. Use{" "}
							<a
								className="doc-link"
								href="https://github.com/sigil-oss/sigil.connect"
					...
					<p>
						Sigil makes an HTTP <code className="inline">POST</code> with a JSON
						body to your callback URL from the Rust layer. If delivery fails,
						the result stays recoverable in request history for retry.
					</p>
					<h3>
						Constraints on <code className="inline">cb</code>
					</h3>
					<ul className="brief">
						<li>
					...
					<pre className="doc-pre">
						...
						<span className="t-str">"rejected"</span>,{"\n"}{" "}
						<span className="t-key">"nonce"</span>:{" "}
						<span className="t-str">
							"{"<"}the nonce you sent{">"}"
						</span>
						...
						,{"\n"} <span className="t-key">"reason"</span>:{" "}
						<span className="t-str">"user_rejected"</span>
						{"\n"}
						{"}"}
					</pre>
					<div className="callout warn">
						...
						<p>
							Match the <code className="inline">nonce</code> on the callback
							against the one you sent. Don't trust the body until that matches.
						</p>
					</div>
					...
					<pre className="doc-pre">
						...
						{"{"}
						{"\n"} <span className="t-key">"type"</span>:{" "}
						<span className="t-str">"connect"</span>,{"\n"}{" "}
						<span className="t-key">"nonce"</span>:{" "}
						<span className="t-str">"a1b2c3d4e5f6g7h8"</span>,{"\n"}{" "}
						<span className="t-key">"dapp"</span>: {"{"}{" "}
						<span className="t-key">"name"</span>:{" "}
						<span className="t-str">"Acme"</span>,{" "}
						<span className="t-key">"origin"</span>:{" "}
						<span className="t-str">"https://acme.example"</span> {"}"},{"\n"}{" "}
						<span className="t-key">"permissions"</span>: [
						<span className="t-str">"transfer"</span>,{" "}
						<span className="t-str">"sign_message"</span>]{"\n"}
						{"}"}
					</pre>
					...
					<pre className="doc-pre">
						...
						<span className="t-str">"transfer"</span>,{"\n"}{" "}
						<span className="t-key">"nonce"</span>:{" "}
						<span className="t-str">"f3a8b2c1..."</span>,{"\n"}{" "}
						<span className="t-key">"dapp"</span>: {"{"}{" "}
						<span className="t-key">"name"</span>:{" "}
						<span className="t-str">"Acme"</span>,{" "}
						<span className="t-key">"origin"</span>:{" "}
						...
						<span className="t-str">"NQZBXKZP4MTLD...UVWXYZK8MF"</span>,{"\n"}{" "}
						<span className="t-key">"amount"</span>:{" "}
						<span className="t-num">1500000</span>
						{"\n"}
						{"}"}
					</pre>
					...
					<pre className="doc-pre">
						...
						{"{"}
						{"\n"} <span className="t-key">"status"</span>:{" "}
						<span className="t-str">"signed"</span>,{"\n"}{" "}
						<span className="t-key">"identity"</span>:{" "}
						<span className="t-str">
							"{"<"}signing identity{">"}"
						</span>
						,{"\n"} <span className="t-key">"signature"</span>:{" "}
						<span className="t-str">
							"{"<"}signature bytes, base64{">"}"
						</span>
						,{"\n"} <span className="t-key">"public_key"</span>:{" "}
						<span className="t-str">
							"{"<"}public key, base64{">"}"
						</span>
						{"\n"}
					...
					<p>
						The official SDK handles envelope construction, ES256 signing, and
						callback parsing so you don't wire up base64url encoding and
						canonicalization yourself.
					</p>
					<pre className="doc-pre">
						<span className="pre-label">INSTALL</span>
...
bun add @sigil-oss/connect`}
					...
					<p>
						...
						. The package re-exports the same Zod schema that Sigil's renderer
						uses, so a request that parses in the SDK is guaranteed to parse
						inside the wallet.
					</p>

					<h2 id="sdk-unsigned">Unsigned requests</h2>
					<p>
						Unsigned requests show a{" "}
						<strong>trust level of "unverified"</strong> in the wallet UI — the
						dApp name and origin are self-reported and can't be verified. Use
						them for prototyping or internal tooling where trust is established
						by other means.
					</p>
					<pre className="doc-pre">
						<span className="pre-label">TRANSFER</span>
						{`import { buildSigilUri } from '@sigil-oss/connect'
...
window.location.href = uri`}
					...
					<pre className="doc-pre">
						<span className="pre-label">SIGN MESSAGE</span>
						{`const uri = buildSigilUri({
  request: {
    type: 'sign_message',
    nonce: crypto.randomUUID(),
  ...
  callback: 'https://acme.example/sigil/callback',
					...
					<pre className="doc-pre">
						<span className="pre-label">CONNECT (REQUEST PERMISSIONS)</span>
						{`const uri = buildSigilUri({
  request: {
    type: 'connect',
    nonce: crypto.randomUUID(),
  ...
  callback: 'https://acme.example/sigil/callback',
					...
					<pre className="doc-pre">
						<span className="pre-label">SC CALL (QEARN LOCK EXAMPLE)</span>
						{`const uri = buildSigilUri({
  request: {
    type: 'sc_call',
    nonce: crypto.randomUUID(),
  ...
  callback: 'https://acme.example/sigil/callback',
					...
					<p>
						Signed requests include an ES256{" "}
						<code className="inline">proof</code> that lets Sigil verify the
						request came from your registered dApp identity. The wallet
					...
					<p>
						The SDK handles this, but here's what happens under the hood so you
						can audit it or implement it without the SDK:
					</p>
					<ul className="brief">
						<li>
							<strong>Canonical payload</strong> = deterministic JSON of{" "}
							<code className="inline">{"{ request, callback }"}</code> with
							keys sorted recursively
						</li>
						<li>
							<strong>payload_hash</strong> = SHA-256 of the canonical payload,
							base64url-encoded (no padding)
						</li>
						<li>
							<strong>signature</strong> = ECDSA P-256 + SHA-256 (
							<code className="inline">SubtleCrypto</code>{" "}
							<code className="inline">ES256</code>) over the canonical payload,
							base64url-encoded
						</li>
						<li>
							Include <code className="inline">public_jwk</code> for
							self-verifying proofs; omit it to rely on registry lookup only
						</li>
					</ul>

					<pre className="doc-pre">
						<span className="pre-label">SIGNED REQUEST (SDK)</span>
						{`import { buildSigilUri, signEnvelope } from '@sigil-oss/connect'
...
window.location.href = buildSigilUri(signed)`}
					...
					<div className="callout warn">
						...
						<p>
							The ES256 private key must never be exposed to the browser. Build
							the signed envelope on your server and return the final{" "}
							<code className="inline">sigil://</code> URI (or just the{" "}
							<code className="inline">d=</code> parameter) to the client. The
							client only opens it.
						</p>
					</div>

					<pre className="doc-pre">
						<span className="pre-label">
							SERVER-SIDE SIGNING (NODE / EXPRESS)
						</span>
						{`import { buildSigilUri, signEnvelope, loadPrivateKey } from '@sigil-oss/connect/node'
...
app.post('/api/sigil/prepare', async (req, res) => {
  ...
  res.json({ uri: buildSigilUri(signed) })
					...
					<p>
						Sigil POSTs a JSON body to your callback URL from its Rust layer.
						The SDK's <code className="inline">parseSigilCallback</code>{" "}
						validates the shape and returns a discriminated union you can switch
						on.
					</p>
					<pre className="doc-pre">
						<span className="pre-label">NODE / EXPRESS CALLBACK HANDLER</span>
						{`import { parseSigilCallback } from '@sigil-oss/connect'
import express from 'express'
...
app.post('/sigil/callback', (req, res) => {
  ...
  res.sendStatus(200)
					...
					<p>
						All callbacks include <code className="inline">status</code>,{" "}
						<code className="inline">nonce</code>, and{" "}
						<code className="inline">type</code>. The remaining fields depend on
						the outcome.
					</p>
  ...
  "reason": "user_rejected"
					...
					<div className="callout info">
						...
						<p>
							...
							<code className="inline">SigilRejectedCallback</code> — all
							derived from the same Zod schema Sigil uses internally.
						</p>
					</div>
					...
					<p>
						Sigil validates every URI in Rust <em>before</em> the renderer sees
						it. Failures are logged to stderr and the request is silently
						dropped — no popup will appear. Check stderr if debugging.
					</p>
					<h3>What Sigil rejects up front</h3>
					<ul className="brief">
						<li>
							Scheme isn't <code className="inline">sigil</code> or host isn't{" "}
							<code className="inline">v1</code>
						</li>
						<li>
							Missing <code className="inline">d</code> parameter, payload over
							8 192 bytes, or invalid base64url
						</li>
						<li>
							Missing <code className="inline">type</code>,{" "}
							<code className="inline">nonce</code>, or{" "}
							<code className="inline">dapp.origin</code>
						</li>
						<li>
							Unknown <code className="inline">type</code>, or nonce outside
							16–128 chars or invalid charset
						</li>
						<li>
							<code className="inline">dapp.origin</code> scheme isn't{" "}
							<code className="inline">https</code>
						</li>
						<li>
							<code className="inline">exp</code> is in the past, or more than 1
							hour from now
						</li>
						<li>
							Callback isn't HTTPS (except{" "}
							<code className="inline">http://localhost</code> /{" "}
							<code className="inline">http://127.0.0.1</code> for local dev)
						</li>
						<li>
							Callback targets a private or loopback address (other than
							localhost)
						</li>
						<li>
							Nonce was used before (replay protection — 1-hour window,
							persisted to disk)
						</li>
						<li>
							Type-specific validation fails (invalid identity format,
					...
					<p>
						Minimal login flow: client builds a signed envelope on the server
						and opens it; server verifies the nonce on callback.
					</p>
					<pre className="doc-pre">
						<span className="pre-label">SERVER — prepare endpoint</span>
						{`// POST /api/sigil/prepare → { uri }
app.post('/api/sigil/prepare', async (req, res) => {
  ...
  res.json({ uri: buildSigilUri(signed) })
					...
					<pre className="doc-pre">
						<span className="pre-label">CLIENT — open Sigil</span>
						{`const { uri } = await fetch('/api/sigil/prepare', { method: 'POST' }).then(r => r.json())
window.location.href = uri   // OS hands it to Sigil`}
					</pre>
					<pre className="doc-pre">
						<span className="pre-label">SERVER — callback handler</span>
						{`app.post('/sigil/callback', (req, res) => {
  const result = parseSigilCallback(req.body)
  ...
  res.sendStatus(200)
					...
					<div className="callout info">
						...
						<p>
							Rust validator:{" "}
							<code className="inline">src-tauri/src/deep_link.rs</code> · Zod
							schema + callback types:{" "}
							<code className="inline">src/lib/request-schema.ts</code> · Trust
							evaluation:{" "}
							<code className="inline">src/lib/request-trust.ts</code> — all
							browseable at{" "}
							<a
								className="doc-link"
