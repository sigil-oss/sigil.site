...
function Feature({ num, title, body, tag, tagVariant = 'default' }: {
  num: string
  title: string
  body: string
  tag: string
  ...
  return (
    <div className="feature">
      ...
      <span className={`tag${tagVariant === 'warn' ? ' warn' : tagVariant === 'info' ? ' info' : ''}`}>{tag}</span>
    </div>
  )
}
    ...
    <div className="wrap">
      <section id="features">
        ...
        <div className="features">
          <Feature num="[ 01 ]" title="Send QUBIC" body="Type an address or pick from your contacts. Confirm the amount. Sign. Done." tag="[ ONE-STEP REVIEW ]" />
        ...
        <div className="features two">
          <Feature num="[ 07 ]" title="Stake with Qearn" body="Lock some QUBIC and earn rewards over time. Sigil shows the lock epoch, maturity date, and projected return before you commit." tag="[ QEARN BUILT-IN ]" tagVariant="info" />
          <Feature num="[ 08 ]" title="Unlock anytime" body="All your locked positions in one list. Mature ones unlock cleanly. Early unlocks warn you about forfeited rewards before you proceed." tag="[ HONEST WARNINGS ]" tagVariant="warn" />
        </div>

        <div className="feature-group-title">[ ORGANIZE ]</div>
        <div className="features">
          <Feature num="[ 09 ]" title="Multiple wallets" body="Personal, savings, side project — separate encrypted wallets with separate passwords. Switch in one click." tag="[ UNLIMITED ]" tagVariant="info" />
          <Feature num="[ 10 ]" title="Multiple accounts" body="Each wallet can hold multiple accounts. One for spending, one for receiving, one for trading — your call." tag="[ COLOR-CODED ]" tagVariant="info" />
          <Feature num="[ 11 ]" title="Privacy mode" body="Hide every balance across every screen with one toggle. Numbers replaced with dots — same character count so nothing leaks." tag="[ ONE-CLICK SHIELD ]" />
        </div>
